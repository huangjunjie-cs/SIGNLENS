...
class OriginGraph extends Component {
  ...
  render() {
    ...
    const nodes = node_list.map((item) => {
      node_dict[item[0]] = {
        x: (item[1][0] * width) / 2 + width / 2,
        y: (item[1][1] * width) / 2 + width / 2,
      };
      return (
        <text
          x={(item[1][0] * width) / 2 + width / 2 - width / 12}
          y={(item[1][1] * width) / 2 + width / 2}
          fontFamily="Verdana"
          fontSize="10"
        >
    ...
    const lines = links.map((item) => {
      ...
      if (item.weight > 0) {
        ...
        return [
          ...
          />,
          <text x={text_pos.x} y={text_pos.y} fill="rgb(0,255,0)" font-size="10">
        ...
        return [
          <line
            ...
            className="line-red"
          />,
          <text x={text_pos.x} y={text_pos.y} fill="rgb(255,0,0)" font-size="10">
            {item.weight.toFixed(1)}
          </text>,
        ];
      } else {
